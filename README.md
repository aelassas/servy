...
If you've ever struggled with the built-in `sc` tool or found NSSM lacking in features or UI, Servy addresses those limitations, including the ability to set a custom working directory so apps behave exactly as if launched from a shortcut or command prompt.

    ...
    public class HelperTests : IDisposable
    {
        ...
        public void WriteFileAtomic_TransientLockReleasedDuringRetry_Succeeds()
        {
            ...
            try
            {
                ...
                // externally observable signal that the first AtomicSecureMove has been attempted.
                // Attribute access is exempt from share-mode checks, so it stays readable and writable
                // through the lock below - the existing FailedMove tests already rely on that.
                File.SetAttributes(targetPath, FileAttributes.ReadOnly);
                ...
                using (var lockStream = new FileStream(targetPath, FileMode.Open, FileAccess.Read, FileShare.None))
                {
                    ...
                    var releaser = new Thread(() =>
                    {
                        try
                        {
                            ...
                            // The attribute is cleared immediately before AtomicSecureMove runs, so wait
                            // that first attempt out: the write has to be proven to succeed on a LATER pass.

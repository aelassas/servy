...
namespace Servy.Manager.UnitTests.Services
{
    public class ServiceCommandsTests : IDisposable
    {
        ...
        public async Task ConfigureServiceAsync_ShouldUseConfiguredPath()
        {
            ...
            // Capture the base file path generated by the OS to ensure we can track and delete it later
            var baseTmpFile = Path.GetTempFileName();
            var tempExe = baseTmpFile + ".exe";
            ...
            {
                // Tear down both the custom appended executable path and the original zero-byte handle file

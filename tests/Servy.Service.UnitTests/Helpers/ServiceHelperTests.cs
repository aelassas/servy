...
namespace Servy.Service.UnitTests.Helpers
{
    ...
    public class ServiceHelperTests
    {
        ...
        public void RestartProcess_StartActionThrows_LogsErrorAndStillDisposesProcess()
        {
            ...
            // The message is matched by prefix because this branch interpolates the resolved path.
            mockProcess.Verify(p => p.Stop(1000), Times.Once);
            mockLog.Verify(l => l.Error(It.Is<string>(s => s.Contains("Failed to restart process")), It.IsAny<InvalidOperationException>()), Times.Once);
            mockProcess.Verify(p => p.Dispose(), Times.Once);
        }
        ...
        /// <summary>
        /// Puts a placeholder Servy.Restarter.Net48.exe where RestartService looks for it, but only when
        ...
        private sealed class PlaceholderRestarter : IDisposable
        {
            ...
            {
                var dir = GetTargetRestarterDirectory();
                _path = Path.Combine(dir, "Servy.Restarter.Net48.exe");

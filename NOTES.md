...
The digital signing integration took some time to set up, as it required rewriting the entire build pipeline to automate code signing using SignPath and GitHub Actions. However, the effort was worthwhile to ensure that Servy is safe and trustworthy for everyone. For reference, here are the new build pipelines:
